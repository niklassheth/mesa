...
# Run the exact rank-2 partial-workgroup fixture through the m1n1 DRM shim.
...
binary=$mesa_build/src/gallium/drivers/asahi/t8132_gallium_compute_xy

if [ "${MESA_SKIP_BUILD:-0}" != 1 ]; then
    ninja -C "$mesa_build" \
        src/gallium/drivers/asahi/t8132_gallium_compute_xy \
