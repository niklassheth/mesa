...
binary=$mesa_build/src/gallium/drivers/asahi/t8132_gallium_compute_device_index

if [ "${MESA_SKIP_BUILD:-0}" != 1 ]; then
    ninja -C "$mesa_build" \
        src/gallium/drivers/asahi/t8132_gallium_compute_device_index \
...
uv run --python 3.14 --with-requirements "$m1n1_root/requirements.txt" sh -c '
    python_site=$(find "$VIRTUAL_ENV/lib" -maxdepth 2 -type d -name site-packages -print -quit)
